...
import kudadiri.DataEngineer.portofolio.config.AvroSerializer;
...
public class KafkaTest {
    ...
    public static void main(String[] args) {
        ...
        KafkaProducer<String, DataRecord> producer = new KafkaProducer<>(props);
        ...
        try(BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            ...
            System.out.println("Hasil dari indexMap = " + indexMap);
            ...
            while((line = br.readLine()) != null && count < 5) {
                ...
                r.setProducerTimestamp(producerTimestamp);
                ...
                r.setProducerTimestampStr(forwardedStr);

                System.out.println("hasil dari Avro record Belum build = " + r);
                System.out.println("hasil dari Avro record sudah build = " + r.build());
                records.add(r.build());
                count++;
            }
...
//        DataRecord data = records.get(0);

        for (DataRecord data : records) {
            ProducerRecord<String, DataRecord> record = new ProducerRecord<>(topic, data);

            final long producerTimestamp = data.getProducerTimestamp();
            final String forwardedStr = data.getProducerTimestampStr().toString();

            System.out.println("hasil dari ProducerRecord = " + record);

            producer.send(record, (recordMetadata, e) -> {
                if (e == null) {
                    long brokerTimestamp = recordMetadata.timestamp();
...
//        var records = CsvReaderService.readCsv(filePath);
//        for (DataRecord record : records) {
//            ProducerRecord<String, DataRecord> kafkaRecord = new ProducerRecord<>("topic-network", record);
